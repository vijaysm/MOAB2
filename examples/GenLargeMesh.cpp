...
int main(int argc, char **argv)
{
  ...
  ReadUtilIface* iface;
  ErrorCode rval = mb->query_interface(iface);CHK_ERR1(rval, "Can't get reader interface");
  ...
  rval = mb->tag_get_handle("GLOBAL_ID", 1, MB_TYPE_INTEGER,
                             global_id_tag);CHK_ERR1(rval, "Can't get global id tag");
  ...
  rval = mb->tag_get_handle("PARALLEL_PARTITION", 1, MB_TYPE_INTEGER,
                             part_tag, MB_TAG_CREAT | MB_TAG_SPARSE, &dum_id);CHK_ERR1(rval, "Can't get parallel partition tag");
  ...
  for (size_t i = 0; i < intTagNames.size(); i++) {
    rval = mb->tag_get_handle(intTagNames[i].c_str(), 1, MB_TYPE_INTEGER, intTags[i],
                              MB_TAG_CREAT | MB_TAG_DENSE, &dum_id);CHK_ERR1(rval, "Can't create integer tag");
  }

  double defval = 0.;
  for (size_t i = 0; i < doubleTagNames.size(); i++) {
    rval = mb->tag_get_handle(doubleTagNames[i].c_str(), 1, MB_TYPE_DOUBLE, doubleTags[i],
                              MB_TAG_CREAT | MB_TAG_DENSE, &defval);CHK_ERR1(rval, "Can't create double tag");
  }
  ...
  for (int a = 0; a < A; a++) {
    for (int b = 0; b < B; b++) {
      for (int c = 0; c < C; c++) {
          ...
          for (int jj = 0; jj < blockSize1; jj++) {
            for (int ii = 0; ii < blockSize1; ii++) {
              ...
              for (size_t i = 0; i < intTags.size(); i++) {
                ...
                rval = mb->tag_set_data(intTags[i], &v, 1, &valv);CHK_ERR1(rval, "Can't set integer tag on a vertex");
              }
              ix++;
            }
          }
        ...
        rval = mb->tag_set_data(global_id_tag, verts, &gids[0]);CHK_ERR1(rval, "Can't set global ids to vertices");
        rval = mb->tag_set_data(new_id_tag, verts, &lgids[0]);CHK_ERR1(rval, "Can't set the new handle id tags");
        ...
        if (quadratic) {
          num_v_per_elem = 27;
          rval = iface->get_element_connect(num_el, 27, MBHEX, 0, starte, conn);CHK_ERR1(rval, "Can't get element connectivity");
        }
        else if (tetra) {
          num_v_per_elem = 4;
          rval = iface->get_element_connect(num_el, 4, MBTET, 0, starte, conn);CHK_ERR1(rval, "Can't get element connectivity");
        }
        else {
          rval = iface->get_element_connect(num_el, 8, MBHEX, 0, starte, conn);CHK_ERR1(rval, "Can't get element connectivity");
        }
  ...
  //
        ...
        rval = mb->create_meshset(MESHSET_SET, part_set);CHK_ERR1(rval, "Can't create mesh set");
        rval = mb->add_entities(part_set, cells);CHK_ERR1(rval, "Can't add entities to set");
        // If needed, add all edges and faces
        if (adjEnts) {
          // We need to update adjacencies now, because some elements are new
          rval = iface->update_adjacencies(starte, num_el, num_v_per_elem, conn);CHK_ERR1(rval, "Can't update adjacencies");
          // Generate all adj entities dimension 1 and 2 (edges and faces/ tri or qua)
          Range edges, faces;
          rval = mb->get_adjacencies(cells, 1, true, edges,
                                     Interface::UNION);CHK_ERR1(rval, "Can't get edges");
          rval = mb->get_adjacencies(cells, 2, true, faces,
                                     Interface::UNION);CHK_ERR1(rval, "Can't get faces");
          rval = mb->add_entities(part_set, edges);CHK_ERR1(rval, "Can't add edges to partition set");
          rval = mb->add_entities(part_set, faces);CHK_ERR1(rval, "Can't add faces to partition set");
        }

        rval = mb->tag_set_data(global_id_tag, cells, &gids[0]);CHK_ERR1(rval, "Can't set global ids to elements");
        rval = mb->tag_set_data(new_id_tag, cells, &lgids[0]);CHK_ERR1(rval, "Can't set new ids to elements");

        int part_num = a + m*A + (b + n*B)*(M*A) + (c + k*C)*(M*A * N*B);
        rval = mb->tag_set_data(part_tag, &part_set, 1, &part_num);CHK_ERR1(rval, "Can't set part tag on set");
      }
    }
  }
  ...
  rval = mb->write_file("test0.h5m", 0, ";;PARALLEL=WRITE_PART");CHK_ERR1(rval, "Can't write in parallel, before merging");
  */
  // After the mesh is generated on each proc, merge the vertices
  MergeMesh mm(mb);
  Range all3dcells;
  rval = mb->get_entities_by_dimension(0, 3, all3dcells);CHK_ERR1(rval, "Can't get all 3d cells elements");

  Range verts;
  rval = mb->get_entities_by_dimension(0, 0, verts);CHK_ERR1(rval, "Can't get all vertices");

  if (A*B*C != 1) { // Merge needed
    if (newMergeMethod) {
      rval = mm.merge_using_integer_tag(verts, global_id_tag);CHK_ERR1(rval, "Can't merge");
    }
    else {
      rval = mm.merge_entities(all3dcells, 0.0001);CHK_ERR1(rval, "Can't merge");
    }
  ...
  if (size > 1) {
    ...
    rval = mb->create_meshset(MESHSET_SET, mesh_set);CHK_ERR1(rval, "Can't create new set");
    mb->add_entities(mesh_set, all3dcells);
    rval = pcomm->resolve_shared_ents(mesh_set, -1, -1, &new_id_tag);CHK_ERR1(rval, "Can't resolve shared ents");
    ...
    if (!keep_skins) { // Default is to delete the 1- and 2-dimensional entities
      ...
      Range toDelete;
      rval = mb->get_entities_by_dimension(0, 1, toDelete);CHK_ERR1(rval, "Can't get edges");

      rval = mb->get_entities_by_dimension(0, 2, toDelete);CHK_ERR1(rval, "Can't get faces");

      rval = pcomm->delete_entities(toDelete);CHK_ERR1(rval, "Can't delete entities");
  ...
  rval = mb->write_file(outFileName.c_str(), 0, ";;PARALLEL=WRITE_PART");CHK_ERR1(rval, "Can't write in parallel");
