...
ErrorCode DeformMeshRemap::execute() 
{
  ...
  if (solidSetNos[MASTER].empty() || fluidSetNos[MASTER].empty()) {
    rval = find_other_sets(MASTER, masterSet);CHK_ERR1(rval, "Failed to find other sets in master mesh");
  }
  ...
  if (have_slave) {
    ...
    if (solidSetNos[SLAVE].empty() || fluidSetNos[SLAVE].empty()) {
      rval = find_other_sets(SLAVE, slaveSet);CHK_ERR1(rval, "Failed to find other sets in slave mesh");
    }
  }
  ...
  if (have_slave) {
    ...
    rval = mbImpl->get_adjacencies(tmp_range, 0, false, tgt_verts, Interface::UNION);CHK_ERR1(rval, "Failed to get target verts");

    // Locate slave vertices, caching results in dc
    if (debug) cout << "Locating slave vertices in master mesh..." << endl;
    rval = dc_master.locate_points(tgt_verts);CHK_ERR1(rval, "Point location of tgt verts failed");
  ...
  { // To isolate the lloyd smoother & delete when done
    if (debug) {
      ...
      Range skin;
      rval = skinner.find_skin(0, fluidElems[MASTER], false, skin);CHK_ERR1(rval, "Unable to find skin");
      EntityHandle skin_set;
      cout << "Writing skin_mesh.g and fluid_mesh.g." << endl;
      rval = mbImpl->create_meshset(MESHSET_SET, skin_set);CHK_ERR1(rval, "Failed to create skin set");
      rval = mbImpl->add_entities(skin_set, skin);CHK_ERR1(rval, "Failed to add skin entities to set");
      rval = mbImpl->write_file("skin_mesh.vtk", NULL, NULL, &skin_set, 1);CHK_ERR1(rval, "Failure to write skin set");
      rval = mbImpl->remove_entities(skin_set, skin);CHK_ERR1(rval, "Failed to remove skin entities from set");
      rval = mbImpl->add_entities(skin_set, fluidElems[MASTER]);CHK_ERR1(rval, "Failed to add fluid entities to set");
      rval = mbImpl->write_file("fluid_mesh.vtk", NULL, NULL, &skin_set, 1);CHK_ERR1(rval, "Failure to write fluid set");
      rval = mbImpl->delete_entities(&skin_set, 1);CHK_ERR1(rval, "Failed to delete skin set");
    }
    ...
    rval = ll.perform_smooth();CHK_ERR1(rval, "Failed in lloyd smoothing");
  ...
  rval = write_to_coords(solidElems[MASTER], xNew);CHK_ERR1(rval, "Failed writing tag to master fluid verts");
  rval = write_to_coords(fluidElems[MASTER], xNew);CHK_ERR1(rval, "Failed writing tag to master fluid verts");
  ...
  if (have_slave) {
    ...
    rval = dc_master.interpolate((int)DataCoupler::VOLUME, "xnew");CHK_ERR1(rval, "Failed to interpolate target solution");
    // Transfer xNew to coords, for slave
    if (debug) cout << "Transferring coords tag to vertex coordinates in slave mesh..." << endl;
    rval = write_to_coords(tgt_verts, xNew);CHK_ERR1(rval, "Failed writing tag to slave verts");
  }
...
ErrorCode DeformMeshRemap::write_to_coords(Range &elems, Tag tagh, Tag tmp_tag) 
{
  ...
  Range verts;
  ErrorCode rval = mbImpl->get_adjacencies(elems, 0, false, verts, Interface::UNION);CHK_ERR1(rval, "Failed to get adj vertices");
  vector<double> coords(3*verts.size());

  if (tmp_tag) {
    // Save the coords to tmp_tag first
    rval = mbImpl->get_coords(verts, &coords[0]);CHK_ERR1(rval, "Failed to get tmp copy of coords");
    rval = mbImpl->tag_set_data(tmp_tag, verts, &coords[0]);CHK_ERR1(rval, "Failed to save tmp copy of coords");
  }

  rval = mbImpl->tag_get_data(tagh, verts, &coords[0]);CHK_ERR1(rval, "Failed to get tag data");
  rval = mbImpl->set_coords(verts, &coords[0]);CHK_ERR1(rval, "Failed to set coordinates");
  return MB_SUCCESS;
}
...
ErrorCode DeformMeshRemap::deform_master(Range &fluid_elems, Range &solid_elems, const char *tag_name) 
{
  ...
  rval = mbImpl->get_adjacencies(solid_elems, 0, false, solid_verts, Interface::UNION);CHK_ERR1(rval, "Failed to get vertices");
  vector<double> coords(3*solid_verts.size()), new_coords(3*solid_verts.size());
  rval = mbImpl->get_coords(solid_verts, &coords[0]);CHK_ERR1(rval, "Failed to get vertex coords");
  ...
  if (!xDispNames[0].empty() && !xDispNames[1].empty() && !xDispNames[2].empty()) {
    ...
    rval = mbImpl->tag_get_handle((tag_name ? tag_name : ""), 3, MB_TYPE_DOUBLE,
        xNew, MB_TAG_CREAT | MB_TAG_DENSE);CHK_ERR1(rval, "Failed to create xnew tag");
    vector<double> disps(num_verts);
    for (int i = 0; i < 3; i++) {
      rval = mbImpl->tag_get_handle(xDispNames[0].c_str(), 1, MB_TYPE_DOUBLE, xDisp[i]);CHK_ERR1(rval, "Failed to get xDisp tag");
      rval = mbImpl->tag_get_data(xDisp[i], solid_verts, &disps[0]);CHK_ERR1(rval, "Failed to get xDisp tag values");
      for (unsigned int j = 0; j < num_verts; j++)
        new_coords[3*j+i] = coords[3*j+i] + disps[j];
    }
  }
  else if (!xDispNames[0].empty()) {
    rval = mbImpl->tag_get_handle(xDispNames[0].c_str(), 3, MB_TYPE_DOUBLE, xDisp[0]);CHK_ERR1(rval, "Failed to get first xDisp tag");
    xNew = xDisp[0];
  ...
  if (!xNew) {
    rval = mbImpl->tag_get_handle((tag_name ? tag_name : ""), 3, MB_TYPE_DOUBLE, 
                                  xDisp[0], MB_TAG_CREAT|MB_TAG_DENSE);CHK_ERR1(rval, "Failed to get xNew tag");
    xNew = xDisp[0];
  }

  // Set the new tag to those coords
  rval = mbImpl->tag_set_data(xNew, solid_verts, &new_coords[0]);CHK_ERR1(rval, "Failed to set tag data");

  // Get all the vertices and coords in the fluid, set xnew to them
  rval = mbImpl->get_adjacencies(fluid_elems, 0, false, fluid_verts, Interface::UNION);CHK_ERR1(rval, "Failed to get vertices");
  fluid_verts = subtract(fluid_verts, solid_verts);

  if (coords.size() < 3*fluid_verts.size()) coords.resize(3*fluid_verts.size());
  rval = mbImpl->get_coords(fluid_verts, &coords[0]);CHK_ERR1(rval, "Failed to get vertex coords");
  rval = mbImpl->tag_set_data(xNew, fluid_verts, &coords[0]);CHK_ERR1(rval, "Failed to set xnew tag on fluid verts");
...
{
  // Create meshset
  ErrorCode rval = mbImpl->create_meshset(0, seth);CHK_ERR1(rval, "Couldn't create master/slave set");
...
#endif  
  rval = mbImpl->load_file(fname.c_str(), &seth, options.str().c_str());CHK_ERR1(rval, "Couldn't load master/slave mesh");

  if (*solidSetNos[m_or_s].begin() == -1 || *fluidSetNos[m_or_s].begin() == -1) return MB_SUCCESS;

  // Get material sets for solid/fluid
  Tag tagh;
  rval = mbImpl->tag_get_handle(MATERIAL_SET_TAG_NAME, tagh);CHK_ERR1(rval, "Couldn't get material set tag name");
  ...
  for (Range::iterator rit = solidSets[m_or_s].begin(); rit != solidSets[m_or_s].end(); ++rit) {
    rval = mbImpl->get_entities_by_handle(*rit, tmp_range, true);CHK_ERR1(rval, "Failed to get entities in solid");
  }
  if (!tmp_range.empty()) {
  ...
  for (Range::iterator rit = fluidSets[m_or_s].begin(); rit != fluidSets[m_or_s].end(); ++rit) {
    rval = mbImpl->get_entities_by_handle(*rit, tmp_range, true);CHK_ERR1(rval, "Failed to get entities in fluid");
  }
  if (!tmp_range.empty()) {
    int dim = mbImpl->dimension_from_handle(*tmp_range.rbegin());
    assert(dim > 0 && dim < 4);
...
ErrorCode DeformMeshRemap::find_other_sets(int m_or_s, EntityHandle file_set) 
{
  ...
  Tag tagh;
  ErrorCode rval = mbImpl->tag_get_handle(MATERIAL_SET_TAG_NAME, tagh);CHK_ERR1(rval, "Couldn't get material set tag name");
  Range matsets;
  ...
  *unfilled_sets = subtract(matsets, *filled_sets);
  ...
  for (Range::iterator rit = unfilled_sets->begin(); rit != unfilled_sets->end(); ++rit) {
    rval = mbImpl->get_entities_by_handle(*rit, tmp_range, true);CHK_ERR1(rval, "Failed to get entities in unfilled set");
  }
  int dim = mbImpl->dimension_from_handle(*tmp_range.rbegin());
  assert(dim > 0 && dim < 4);  
