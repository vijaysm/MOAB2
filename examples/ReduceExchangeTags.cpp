...
int main(int argc, char **argv)
{
  ...
  err = get_file_options(argc, argv, filename, tagName, tagValue);CHK_SET_ERR(err, "get_file_options failed");
  ...
  err = mbi->create_meshset(MESHSET_SET, rootset);CHK_SET_ERR(err, "Creating root set failed");
  err = mbi->create_meshset(MESHSET_SET, partnset);CHK_SET_ERR(err, "Creating partition set failed");
  ...
  err = mbi->load_file(filename.c_str(), &rootset, read_options.c_str());CHK_SET_ERR(err, "MOAB::load_file failed");
  ...
  Tag tagReduce, tagExchange;
  {
    ...
    err = mbi->tag_get_handle(sstr.str().c_str(), 1, MB_TYPE_INTEGER, tagExchange,
        MB_TAG_CREAT|MB_TAG_DENSE, &tagValue);CHK_SET_ERR(err, "Retrieving tag handles failed");
  ...
  dbgprint("-Exchanging tags between processors ");
  {
    ...
    for (int dim = 0; dim <= 3; dim++) {
      ...
      err = mbi->get_entities_by_dimension(rootset, dim, dimEnts, false);CHK_ERR(err);
      ...
      err = mbi->tag_set_data(tagExchange, dimEnts,
          &tagValues[0]);CHK_SET_ERR(err, "Setting local tag data failed during exchange phase");
      // Merge entities into parent set
      partEnts.merge(dimEnts);
    }

    // Exchange tags between processors
    err = parallel_communicator->exchange_tags(tagExchange, partEnts);CHK_SET_ERR(err, "Exchanging tags between processors failed");
  }
  ...
  dbgprint("-Reducing tags between processors ");
  {
    ...
    err = parallel_communicator->get_part_entities(partEnts);CHK_SET_ERR(err, "ParallelComm::get_part_entities failed");
    ...
    err = mbi->tag_set_data(tagReduce, partEnts,
        &tagValues[0]);CHK_SET_ERR(err, "Setting local tag data failed during reduce phase");

    Range dummy;
    // Reduce tag data using MPI_SUM on the interface between partitions
    err = parallel_communicator->reduce_tags(tagReduce, MPI_SUM,
        dummy/*partEnts*/);CHK_SET_ERR(err, "Reducing tags between processors failed");
  }
  // Write out to output file to visualize reduction/exchange of tag data
  err = mbi->write_file("test.h5m", "H5M", "PARALLEL=WRITE_PART");CHK_ERR(err);
