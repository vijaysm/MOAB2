...
int main(int argc, char **argv)
{
  ...
  ErrorCode rval = create_mesh_with_holes(mbImpl, nquads, nholes);CHK_ERR1(rval, "Trouble creating mesh");

  // Get all vertices and non-vertex entities
  Range verts, elems;
  rval = mbImpl->get_entities_by_handle(0, elems);CHK_ERR1(rval, "Trouble getting all entities");
  verts = elems.subset_by_dimension(0);
  elems -= verts;

  // Create tag1 (element-based avg), tag2 (vertex-based avg), tag3 (# connected verts)
  Tag tag1, tag2, tag3;
  rval = mbImpl->tag_get_handle("tag1", 3, MB_TYPE_DOUBLE, tag1, MB_TAG_DENSE | MB_TAG_CREAT);CHK_ERR1(rval, "Trouble creating tag1");
  double def_val[3] = {0.0, 0.0, 0.0}; // Need a default value for tag2 because we sum into it
  rval = mbImpl->tag_get_handle("tag2", 3, MB_TYPE_DOUBLE, tag2, MB_TAG_DENSE | MB_TAG_CREAT, def_val);CHK_ERR1(rval, "Trouble creating tag2");
  int def_val_int = 0;  // Need a default value for tag3 because we increment it
  rval = mbImpl->tag_get_handle("tag3", 1, MB_TYPE_INTEGER, tag3, MB_TAG_DENSE | MB_TAG_CREAT, &def_val_int);CHK_ERR1(rval, "Trouble creating tag3");
  ...
  rval = mbImpl->coords_iterate(verts.begin(), verts.end(), x_ptr, y_ptr, z_ptr, count);CHK_ERR1(rval, "Error in coords_iterate");
  ...
  while (e_it != elems.end()) {
    ...
    rval = mbImpl->connect_iterate(e_it, elems.end(), conn_ptr, vpere, count);CHK_ERR1(rval, "Error in connect_iterate");
    rval = mbImpl->tag_iterate(tag1, e_it, elems.end(), count, (void*&)tag1_ptr);CHK_ERR1(rval, "Error in tag1_iterate");
  ...
  while (e_it != elems.end()) {
    ...
    rval = mbImpl->tag_iterate(tag2, e_it, elems.end(), count, (void*&)ts.avg_ptr);CHK_ERR1(rval, "Error in tag2_iterate");
    rval = mbImpl->tag_iterate(tag3, e_it, elems.end(), count, (void*&)ts.nv_ptr);CHK_ERR1(rval, "Error in tag3_iterate");
    elem_map[*e_it] = ts;
    e_it += count;
  }
  ...
  Range dum_range;
  rval = mbImpl->get_adjacencies(&(*v_it), 1, 2, false, dum_range);CHK_ERR1(rval, "Error in get_adjacencies");
  ...
  while (v_it != verts.end()) {
    ...
    rval = mbImpl->coords_iterate(v_it, verts.end(), x_ptr, y_ptr, z_ptr, count);CHK_ERR1(rval, "Error in coords_iterate");
    rval = mbImpl->adjacencies_iterate(v_it, verts.end(), adjs_ptr, count);CHK_ERR1(rval, "Error in adjacencies_iterate");
  ...
  while (e_it != elems.end()) {
    // Get conn_ptr, tag1_ptr for next contiguous chunk of element handles, and coords pointers for all verts
    rval = mbImpl->tag_iterate(tag1, e_it, elems.end(), count, (void*&)tag1_ptr);CHK_ERR1(rval, "Error in tag1_iterate");
    rval = mbImpl->tag_iterate(tag2, e_it, elems.end(), count, (void*&)tag2_ptr);CHK_ERR1(rval, "Error in tag2_iterate");
    rval = mbImpl->tag_iterate(tag3, e_it, elems.end(), count, (void*&)tag3_ptr);CHK_ERR1(rval, "Error in tag3_iterate");
...
ErrorCode create_mesh_with_holes(Interface *mbImpl, int nquads, int nholes) 
{
  ...
  ErrorCode rval = mbImpl->query_interface(read_iface);CHK_ERR1(rval, "Error in query_interface");
  ...
  rval = read_iface->get_node_coords(3, 2*(nquads + 1), 0, start_vert, coords);CHK_ERR1(rval, "Error in get_node_arrays");
  // Create elems
  rval = read_iface->get_element_connect(nquads, 4, MBQUAD, 0, start_elem, connect);CHK_ERR1(rval, "Error in get_element_connect");
  ...
  for (int i = 0; i < nholes; i++) {
    start_elem += de;
    rval = mbImpl->delete_entities(&start_elem, 1);CHK_ERR1(rval, "Error in delete_entities");
  }
