#ifndef MATRIX_H
#define MATRIX_H
//URL: http://www.rejonesconsulting.com/indexlibrary.htm
...
Matrix Matrix::operator* (const Matrix &B) const
{
  ...
  for (int i = 0; i < m_; i++)
    ...
    {
      dsum = 0.0;
      for (int k = 0; k < n_; k++) dsum += v_[i][k] * B[k][j];
      C[i][j] = dsum;
    }
  return C;
}
...
T Matrix::maxval() const
{
  ...
  T tl = data_[0];
  for (int i = 1; i < sz; i++) { if (data_[i] > tl) tl = data_[i]; }
  return tl;
}
...
T Matrix::maxabs() const
{
  ...
  T tl = absf(data_[0]);
  for (int i = 1; i < sz; i++) { if (absf(data_[i]) > tl) tl = absf(data_[i]); }
  return tl;
}
...
T Matrix::minpos() const
{
  ...
  if (sz < 1) return 0.0;
  T tl = maxabs();
  for (int i = 0; i < sz; i++) { if (data_[i] > 0.0 && data_[i] < tl) tl = data_[i]; }
  return tl;
}
...
void Matrix::ijmaxabs(I &imax, I &jmax) const
{
  ...
  for (int i = 0; i < m_; i++)
    for (int j = 0; j < n_; j++)
      if (absf(v_[i][j]) > tl) { tl = absf(v_[i][j]); imax = i; jmax = j; }
}
...
T Matrix::sumabs() const
{
  ...
  if (sz < 1) return 0.0;
  T tl = 0.0;
  for (int i = 0; i < sz; i++) { tl += absf(data_[i]); }
  return tl;
}
...
T Matrix::norm2_as_vector() const
{
  I mn = m_ * n_;
  T dsum = 0.0;
  for (int i = 0; i < mn; i++) { dsum += data_[i] * data_[i]; }
  return dsum;
}
...
{
  T dsum = 0.;
  for (int j = 0; j < n_; j++) dsum += v_[i][j] * v_[k][j];
  return dsum;
}
...
T Matrix::dot (Matrix &B) const
{
  ...
  T dsum = 0.0;
  for (I i = 0; i < amn; i++) dsum += data_[i] * B.data_[i];
  return dsum;
}
...
void Matrix::printA() const
{
  ...
  for (int i = 0; i < mx; i++)
  {
    ...
    {
      tl = v_[i][j];
      if (absf(tl) < small) tl = 0.0;
      cout << setw(10) << setprecision(4) <<  tl    << " ";
    }
    if (n > nn) cout << "...";
    cout << endl;
  }
...
public:
  ...
  //transposes *this
  void transpose() { I tl = n_; n_ = m_; m_ = tl; }
...
I Vector::imax() const
{
  ...
  T tl = data_[0]; I k = 0;
  for (int i = 1; i < sz; i++) { if (data_[i] > tl) { tl = data_[i]; k = i; } }
  return k;
}
...
I Vector::imaxabs() const
{
  I sz = m_ * n_;
  if (sz < 1) Matrix::xerror(3, "Matrix::imaxabs");
  T tl = absf(data_[0]); I k = 0;
  for (int i = 1; i < sz; i++) { if (absf(data_[i]) > tl) { tl = absf(data_[i]); k = i; } }
  return k;
}
...
Vector Vector::moving_average(I w)
{
  ...
  {
    dsum = 0.0;
    for (int j = i; j < i + w; j++) dsum += data_[j];
    avg[i] = dsum;
  }
  return avg;
}
...
Vector Vector::moving_average_fast(I w)
{
  ...
  //do first sum
  T dsum = 0.0;
  for (int j = 0; j < w; j++) dsum += data_[j];
  avg[0] = dsum;

  //do subsequent sum adjustments
  for (int i = 1; i < ln; i++)
  {
    dsum = dsum - data_[i - 1] + data_[i + w - 1];
    avg[i] = dsum;
  }
  return avg;
}
