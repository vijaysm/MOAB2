  ...
  */
  ...
  ErrorCode ParallelComm::augment_default_sets_with_ghosts( EntityHandle file_set)
  {
    ...
    rval = mbImpl->tag_get_handle(GLOBAL_ID_TAG_NAME, 1, MB_TYPE_INTEGER,
        tags[num_tags], MB_TAG_ANY);MB_CHK_SET_ERR(rval, "can't get global id tag");
    ...
    rval = get_shared_entities(-1, owned_shared_ents, -1,  false, true); MB_CHK_SET_ERR(rval, "can't get shared entities");
    ...
    int ir=0, jr=0;
    for (Range::iterator eit=owned_shared_ents.begin(); eit!=owned_shared_ents.end(); eit++)
    {
      // ghosted eh
