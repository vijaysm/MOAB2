...
namespace moab{
  ...
  ErrorCode NestedRefine::estimate_hm_storage(int *level_degrees, int num_level, int hmest[][4])
  {
    ErrorCode error;
    ...
    error = ahf->count_subentities(ahf->_faces, ahf->_cells, &nedges, &nfaces);

    int findex = mb->type_from_handle(*(ahf->_faces.begin()))-1;
    int cindex = mb->type_from_handle(*(ahf->_cells.begin()))-1;
    ...
    hmest[0][0] = nverts_in + nverts;
    hmest[0][1] = nedges_in*refTemplates[MBEDGE-1][level_degrees[0]].total_new_ents;
    hmest[0][2] = nfaces_in*refTemplates[findex][level_degrees[0]].total_new_ents;
    hmest[0][3] = ncells_in*refTemplates[cindex][level_degrees[0]].total_new_ents;

    int nverts_prev, nedges_prev, nfaces_prev, ncells_prev;
    for (int l=1; l<num_level; l++)
      {
        nverts_prev = hmest[l-1][0];
        nedges_prev = hmest[l-1][1];
        nfaces_prev = hmest[l-1][2];
        ncells_prev = hmest[l-1][3];
        ...
        hmest[0][0] = nverts_in + nverts;
        hmest[0][1] = nedges_prev*refTemplates[MBEDGE-1][level_degrees[l]].total_new_ents;
        hmest[0][2] = nfaces_prev*refTemplates[findex][level_degrees[l]].total_new_ents;
        hmest[0][3] = ncells_prev*refTemplates[cindex][level_degrees[l]].total_new_ents;
      }
  ...
  ErrorCode NestedRefine::create_hm_storage(EntityHandle set, int cur_level, int estL[4])
  {
    ...
    error = mb->add_entities(set, *level_mesh[cur_level].start_vertex, estL[0]);
    if (error != MB_SUCCESS) return error;
    ...
    if (estL[1])
      {
        ...
        error = mb->add_entities(set, level_mesh[cur_level].start_edge, estL[1]);
        if (error != MB_SUCCESS) return error;
      }
    ...
    if (estL[2])
      {
        EntityType type = mb->type_from_handle(*(ahf->_faces.begin()));
        int nvpf = ahf->local_maps_2d(*(ahf->_faces.begin()));
        ...
        error = mb->add_entities(set, level_mesh[cur_level].start_face, estL[2]);
        if (error != MB_SUCCESS) return error;
      }
    ...
    if (estL[3])
      {
        EntityType type = mb->type_from_handle(*(ahf->_cells.begin()));
        int index = ahf->get_index_from_type(ahf->_cells.begin());
        ...
        error = mb->add_entities(set, level_mesh[cur_level].start_cell, estL[3]);
        if (error != MB_SUCCESS) return error;
      }
  ...
  ErrorCode NestedRefine::generate_hm(int *level_degrees, int num_level, int hmest[][4], EntityHandle *hm_set)
  {
    ErrorCode error;

    //EntityHandle * set_label= new EntityHandle[num_level];

    for (int l = 0; l<num_level; l++)
      {
       // set_label[l] = l;
        //Create arrays for storing the current level
        error = create_hm_storage(hm_set[l], l, hmest[l][4]);
        if (error != MB_SUCCESS) return error;
  ...
  ErrorCode NestedRefine::construct_hm_entities(int cur_level, int deg)
  {
    ...
    //Step 1: Create a buffer for vertices. This buffer is used during mesh refinement to
    // store the new vertices for the current entity to be refined.
    int nvpf, nvpc, nwverts, vtotal;
    if (ahf->thismeshtype == CURVE)
        vtotal = 2 + refTemplates[MBEDGE-1][deg].total_new_verts;

    else if(ahf->thismeshtype == SURFACE || ahf->thismeshtype == SURFACE_MIXED)
      {
        int findex = mb->type_from_handle(*(ahf->_faces.begin()))-1;
        nwverts = refTemplates[findex][deg].total_new_verts;
        nvpf = ahf->local_maps_2d(ahf->_faces.begin());
        vtotal = nvpf + nwverts;
      }
    else
      {
        int cindex = mb->type_from_handle(*(ahf->_cells.begin()))-1;
        int nwverts = refTemplates[cindex][deg].total_new_verts;
        int index = ahf->get_index_from_type(ahf->_cells.begin());
    ...
    //Step 2: Generate mesh for current level by refining previous level.
    if (ahf->thismeshtype == CURVE)
      {
        error = construct_hm_1D(cur_level, deg, vbuffer);
        if (MB_SUCCESS != error) return error;
      }
     else if(ahf->thismeshtype == SURFACE || ahf->thismeshtype == SURFACE_MIXED)
      {
        error = construct_hm_2D(cur_level, deg, vbuffer);
        if (MB_SUCCESS != error) return error;
      }
    else
      {
        error = construct_hm_3D(cur_level, deg, vbuffer);
        if (MB_SUCCESS != error) return error;
      }
  ...
  ErrorCode NestedRefine::construct_hm_1D(int cur_level, int deg, EntityHandle *vbuffer)
  {
    ...
    for (int eid = 0; eid < level_mesh[cur_level-1].num_edges; eid++)
      {
        conn.clear();

        // EntityHandle of the working edge
        EntityHandle edge = level_mesh[cur_level-1].start_edge + eid;
        ...
        for (int i=0; i<num_new_verts; i++)
            vbuffer[i+conn.size()] = level_mesh[cur_level].start_vertex+nverts_prev+i;
        ...
        for (int i=0; i< num_new_verts; i++)
          {
            ...
            idx =  level_mesh[cur_level].start_vertex+nverts_prev+i; // index of new vertex in current level
            ...
            level_mesh[cur_level].coordinates[0][idx] = xi*level_mesh[cur_level].coordinates[0][id1] + (1-xi)*level_mesh[cur_level].coordinates[0][id2];
            level_mesh[cur_level].coordinates[1][idx] = xi*level_mesh[cur_level].coordinates[1][id1] + (1-xi)*level_mesh[cur_level].coordinates[1][id2];
            level_mesh[cur_level].coordinates[2][idx] = xi*level_mesh[cur_level].coordinates[2][id1] + (1-xi)*level_mesh[cur_level].coordinates[2][id2];
          }
      }
    return MB_SUCCESS;
  }
  ...
  ErrorCode NestedRefine::construct_hm_2D(int cur_level, int deg, EntityHandle *vbuffer)
  {
    ...
    int nepf = ahf->local_maps_2d(ahf->_faces.begin());
    EntityType type = mb->type_from_handle(*(ahf->_faces.begin()));
    int findex = type-1;
  ...
  //  EntityHandle *trackvertsF = new EntityHandle[nepf*nve*nents_prev];
  //  int *flag_ents = new  int[nents_prev];
    ...
    for (int fid = 0; fid < nents_prev; fid++)
      {        
        ...
        EntityHandle face = level_mesh[cur_level-1].start_face + fid;
        ...
        error = update_sibhf_map(cur_level, deg, type, ent_buffer, etotal);
        if (error != MB_SUCCESS) return error;
            ...
            for (int j = 0; j < nve; j++)
              {
                ...
                error = ahf->get_up_adjacencies_2d(face, i, 0, sib_fids, true,  sib_leids, true, sib_orient);
                if (error != MB_SUCCESS) return error;
                    ...
                    if (sib_orient[s]) // Same half-edge direction as the current half-edge
                      {
                        ...
                        trackvertsF[sib_fid[s]*nve*nepf+nve*sib_leids[s]+j] = vbuffer[id];
                      }
                    else
                      {
        ...
        error = get_coordinates(conn, nc, cur_level, corner_coords);
        if (error != MB_SUCCESS) return error;

        int vtotal = refTemplates[findex][deg].total_new_verts;
  ...
  ErrorCode NestedRefine::construct_hm_3D(int cur_level, int deg, EntityHandle *vbuffer)
  {
    ...
    std::vector<int> flag_ents(nents_prev, 0);
  //  EntityHandle *trackvertsC_edg = new EntityHandle[nepc*ne*nents_prev];
 //   EntityHandle *trackvertsC_face = new EntityHandle[nfpc*nvf*nents_prev];
   // int *flag_ents = new  int[nents_prev];
    ...
    for (int cid = 0; cid < nents_prev; cid++)
      {
        ...
        for (int i=0; i<conn.size(); i++)
          {
            if (cur_level)
              ...
              vbuffer[i] = (conn[i]-ahf->_verts.begin()) + level_mesh[0].start_vertex;
          }
        ...
        //Use the template to obtain the subentities
        error = subdivide_cell(cell, conn, cur_level, deg, vbuffer);
        if (error != MB_SUCCESS) return error;

        //Update tracking information
        error = update_tracking_verts(cell, deg, trackvertsC_edg, trackvertsC_face, vbuffer);
        if (error != MB_SUCCESS) return error;
      }
  ...
  ErrorCode NestedRefine::update_tracking_verts(EntityHandle cid, int deg, std::vector<EntityHandle> trackvertsC_edg, std::vector<EntityHandle> trackvertsC_face, EntityHandle *vbuffer)
  {
    ErrorCode error;
    ...
    EntityType cell_type = mb->type_from_handle(*(ahf->_cells.begin()));
    ...
    for (int i=0; i<nepc; i++)
      {
        error = ahf->get_up_adjacencies_edg_3d(cid, i, inc_cids, true, inc_leids, inc_orient);
        if (error != MB_SUCCESS) return error;
        ...
        for (int j=0; j< nve; j++)
          {
            ...
            trackvertsC_edg[cid*ne*nepc+ne*i+j] = vbuffer[id];
    ...
    if (nvf) {
        ...
        for (int i=0; i< nfpc; i++)
          {
            error = ahf->get_up_adjacencies_face_3d(cid, i, sib_cids, true, sib_lfids);
            if (error != MB_SUCCESS) return error;

            if (sib_cids[0]) {
                int *id_sib = new int[nvf];
                error = match_and_reorder_vertices(cell_type, cid, i, sib_cids[0], sib_lfids[0], id_sib);
              }
  ...
  ErrorCode NestedRefine::subdivide_cells(EntityHandle cell, EntityType type, std::vector<EntityHandle> conn, int cur_level, int deg, EntityHandle *vbuffer, int *count)
  {
    ...
    if (type == MBTET)
      {
        // Subdivide the tet
        error = subdivide_tets(cell, conn, cur_level, deg, vbuffer, count);
        if (error != MB_SUCCESS) return error;
      }
    else {
        // Connectivity
        ...
        int vtotal = refTemplates[cindex][deg].total_new_verts;
        error = compute_coordinates(cur_level, deg, type, vbuffer, vtotal, corner_coords);
        if (error != MB_SUCCESS) return error;
      }
  ...
  ErrorCode NestedRefine::subdivide_tets(std::vector<EntityHandle> conn, int cur_level, int deg, EntityHandle *vbuffer, int *count)
  {
    ...
    int nents = refTemplates[cindex][deg].num_ents;
    ...
    for (int i=0; i<nents; i++)
      {
        for (int j=0; j<8; j++)
          {
            mixed_rep[i][j] = vbuffer[refTemplates[cindex][deg].ents_conn[i][j]];
...
//    int count=0;
            ...
            for (int j=0; j<4; j++)
              {
               for (int k=0; k<4; k++)
                 {
                   id = oct_tessellation[diag].ent_conn[j][k];
    ...
    error = update_local_ahf(deg, nents_flag, idx_buffer, ent_buffer, count-1);
    if (error != MB_SUCCESS) return error;
...
ErrorCode NestedRefine::update_local_ahf(int deg, EntityType type, EntityHandle *ent_buffer, int etotal)
{
  ...
  if (type == MBEDGE)
    ...
    nhf = ahf->local_maps_2d(ahf->_faces.begin());
  else
    {
      int index = ahf->get_index_from_type(ahf->_cells.begin());
      ...
      for (int lf = 0; lf < nhf; lf++)
        {
          ...
          int id = refTemplates[type-1][deg].ents_opphfs[i][2*j];
          sib_entids[j] = ent_buffer[id];
          sib_lids[j] = refTemplates[type-1][deg].ents_opphfs[i][2*j+1];
        }
...
ErrorCode NestedRefine::update_local_ahf(int deg, std::vector<int> nents_flag, std::vector<int> idx_buffer)
{
  ErrorCode error;
...
/*ErrorCode NestedRefine::update_ahf_maps(int cur_level, int deg, EntityType type, EntityHandle parent, EntityHandle *ent_buffer, int etotal)
{
  if (type == MBEDGE)
    {
...
ErrorCode NestedRefine::get_sibling_tag(EntityType type, std::vector<EntityHandle> ents, int num_ents, std::vector<EntityHandle> sib_entids, std::vector<int> sib_lids)
{
  ErrorCode error;

  if (type == MBEDGE)
    {
      error = mb->tag_get_data(ahf->sibhvs_eid, &*ents, num_ents, sib_entids);
      if (MB_SUCCESS != error) return error;

      error = mb->tag_get_data(ahf->sibhvs_lvid, &*ents, num_ents, sib_lids);
      if (MB_SUCCESS != error) return error;
    }
  else if (type == MBTRI || type == MBQUAD)
    {
      error = mb->tag_get_data(ahf->sibhes_fid, &*ents, num_ents, sib_entids);
      if (MB_SUCCESS != error) return error;

      error = mb->tag_get_data(ahf->sibhes_leid, &*ents, num_ents, sib_lids);
      if (MB_SUCCESS != error) return error;
    }
  else
    {
      error = mb->tag_get_data(ahf->sibhfs_cid, &*ents, num_ents, sib_entids);
      if (MB_SUCCESS != error) return error;

      error = mb->tag_get_data(ahf->sibhfs_lfid, &*ents, num_ents, sib_lids);
      if (MB_SUCCESS != error) return error;
    }
  return MB_SUCCESS;
}

ErrorCode NestedRefine::set_sibling_tag(EntityType type, std::vector<EntityHandle> ents, int num_ents, std::vector<EntityHandle> set_entids, std::vector<int> set_lids)
{
  ErrorCode error;
  if (type == MBEDGE)
    {
      error = mb->tag_set_data(ahf->sibhvs_eid, &*ents, num_ents, set_entids);
      if (MB_SUCCESS != error) return error;

      error = mb->tag_set_data(ahf->sibhvs_lvid, &*ents, num_ents, set_lids);
      if (MB_SUCCESS != error) return error;
    }
  else if (type == MBTRI || type == MBQUAD)
    {
      error = mb->tag_set_data(ahf->sibhes_fid, &*ents, num_ents, set_entids);
      if (MB_SUCCESS != error) return error;

      error = mb->tag_set_data(ahf->sibhes_leid, &*ents, num_ents, set_lids);
      if (MB_SUCCESS != error) return error;
    }
  else
    {
      error = mb->tag_set_data(ahf->sibhfs_cid, &*ents, num_ents, set_entids);
      if (MB_SUCCESS != error) return error;

      error = mb->tag_set_data(ahf->sibhfs_lfid, &*ents, num_ents, set_lids);
      if (MB_SUCCESS != error) return error;
    }
  return MB_SUCCESS;
}
...
ErrorCode NestedRefine::match_and_reorder_vertices(EntityType type, int cur_level, int deg, EntityHandle cell, int lfid, EntityHandle sib_cell, int sib_lfid, int *id_sib)
{
  ...
  ErrorCode error;
  int index = ahf->get_index_from_type(ahf->_cells.begin());
  int nvF = ahf->lConnMap3D[index].hf2v_num[lfid];
  ...
  if (deg == 3)
    {
      ...
      for (int i=0; i<nvF; i++)
        {
          int index = ahf->get_index_from_type(ahf->_cells.begin());
          int id = ahf->lConnMap3D[index].hf2v[lfid][i];
          lface[i] = cell_conn[id];

          int idx = ahf->lConnMap3D[index].hf2vs[sib_lfid][i];
...
/*int NestedRefine::find_permutation(EntityType type, std::vector<EntityHandle> cell_conn, int lfid, std::vector<EntityHandle> sibcell_conn, int sib_lfid)
{
  int nvF = ahf->lConnMap3D[index].hf2v_num[lfid];
  if (nvF == 3)
    {
