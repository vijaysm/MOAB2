...
ErrorCode ReadNC::create_verts_quads(ScdInterface *scdi, EntityHandle tmp_set, Range &quads) 
{
  Range tmp_range;
  ScdBox *scd_box;
  bool is_i_periodic = false;
  if ((iPeriodic) && ((gDims[3]-gDims[0]) == (lDims[3]-lDims[0])))
    is_i_periodic = true;
...
{      
  ...
  if (var_names.empty()) {
    for (mit = varInfo.begin(); mit != varInfo.end(); mit++) {
      ...
      bool has_k = false;
      if (-1 != lCDims[2] && 
	  std::find(vd.varDims.begin(), vd.varDims.end(), kCDim) != vd.varDims.end()) { 
	tmp_v.push_back(kCDim); has_k = true; }     
      tmp_v.push_back(jCDim);
      tmp_v.push_back(iCDim);
      // only variables have all lev lat lon are read for now
      if ((tmp_v == vd.varDims) && has_k)
        vdatas.push_back(vd);
    }
  }
  else {
  ...
  for (unsigned int i = 0; i < vdatas.size(); i++) {
    for (unsigned int t = 0; t < tstep_nums.size(); t++) {
      ...
      vdatas[i].readCounts[t].push_back(1);
        ...
        // then z/y/x
      // assume the kDim and kCDim is the same for now
      bool have_k = false;
      if (std::find(vdatas[i].varDims.begin(), vdatas[i].varDims.end(), kCDim) != vdatas[i].varDims.end()) {
	vdatas[i].readDims[t].push_back(lCDims[2]); 
	vdatas[i].readCounts[t].push_back(lCDims[5] - lCDims[2] + 1);
	have_k = true;
      }
      ...
      if (!have_k && -1 != kCDim && 0 != myPcomm->proc_config().proc_rank())
        have_ij = false;
#else
      ...
      vdatas[i].readCounts[t].push_back(have_ij ? lCDims[4]-lCDims[1]+1 : 0);
      vdatas[i].readCounts[t].push_back(have_ij ? lCDims[3]-lCDims[0]+1 : 0);
      ...
      void *data;
      int count;
      rval = mbImpl->tag_iterate(vdatas[i].varTags[t], quads.begin(), quads.end(), count, data);
      ERRORR(rval, "Failed to get tag iterator.");
      assert((unsigned)count == quads.size());
                                 ...
                                 std::vector<int> &tstep_nums) 
{
  ...
  rval = read_variable_allocate(vdatas, tstep_nums, quads);
  ...
  for (unsigned int i = 0; i < vdatas.size(); i++) {
	...
	std::vector<float> tmpdata(sz);
	success = NCFUNCAG(_vara_float)(fileId, vdatas[i].varId, 
					&vdatas[i].readDims[t][0], &vdatas[i].readCounts[t][0], 
					&tmpdata[0] NCREQ); //(float*)data NCREQ);
	std::size_t num_k = vdatas[i].readCounts[t][1];
	...
	for (std::size_t tj = 0; tj != num_j; ++tj)
	  for (std::size_t ti = 0; ti != num_i; ++ti) {
	    ...
	    for (std::size_t l = 0; l != num_k; ++l) 
	      ((float*)data)[pos+l] = tmpdata[l*num_ij+tj*num_i+ti];	      
	  }
  ...
  switch (var_data.varDataType) {
    case NC_FLOAT:
        ...
        fdata = (float*)var_data.varDatas[tstep_num];
          // convert in-place
        for (int i = quads.size()*(gCDims[5]-gCDims[2]+1)-1; i >= 0; i--) 
          ddata[i] = fdata[i];
        break;
    case NC_SHORT:
  ...
  if (2 <= dbgOut.get_verbosity() && !success) {
    ...
    switch (var_data.varDataType) {
      ...
      case NC_FLOAT:
          ...
          dmin = dmax = ddata[0];
          for (unsigned int i = 1; i < quads.size()*(gCDims[5]-gCDims[2]+1); i++) {
...
ErrorCode ReadNC::get_tag(VarData &var_data, int tstep_num, Tag &tagh) 
{
  ...
  switch (var_data.varDataType) {
    case NC_BYTE:
    case NC_CHAR:
      rval = mbImpl->tag_get_handle(tag_name.str().c_str(), gCDims[5]-gCDims[2]+1, MB_TYPE_OPAQUE, tagh, MB_TAG_DENSE|MB_TAG_CREAT);
      break;
    case NC_DOUBLE:
    case NC_FLOAT:
      rval = mbImpl->tag_get_handle(tag_name.str().c_str(), gCDims[5]-gCDims[2]+1, MB_TYPE_DOUBLE, tagh, MB_TAG_DENSE|MB_TAG_CREAT);
      break;
    case NC_INT:
    case NC_SHORT:
      rval = mbImpl->tag_get_handle(tag_name.str().c_str(), gCDims[5]-gCDims[2]+1, MB_TYPE_INTEGER, tagh, MB_TAG_DENSE|MB_TAG_CREAT);
      break;
    default:
...
ErrorCode ReadNC::init_FVCDscd_vals(const FileOptions &opts, ScdInterface *scdi, EntityHandle file_set) 
{
  ...
  iDim = idx;
  gDims[3] = dimVals[idx]-1+1;
  gDims[0] = 0;
  iName = dimNames[idx];

  // check and set iPeriodic
  std::vector<double> tilVals(2);
  ErrorCode rval = read_coordinate(iName.c_str(), dimVals[idx]-2, dimVals[idx]-1, tilVals);
  ERRORR(rval, "Trouble reading slon variable.");    
  if ((2*tilVals[1] - tilVals[0]) == 360)
    iPeriodic = true;
  ...
  // ilev actually has the number of interfaces which is what we want.  
  gDims[2] = gDims[5] = -1;
  ...
  // look for names of center i/j/k dimensions
  ...
  // assume the center k dimension is the same as k dimension
  if ((vit = std::find(dimNames.begin(), dimNames.end(), "lev")) != dimNames.end())
    idx = vit-dimNames.begin();
  else ERRORR(MB_FAILURE, "Couldn't find lev variable.");
  gCDims[5] = dimVals[idx]-1;
  gCDims[2] = 0;
  kCName = dimNames[idx];
  kCDim = idx;
  ...
  // create grid specific coordinate dimension tags
  // <dim_name>
  std::string tag_name;
  for (unsigned int i = 0; i != dimNames.size(); ++i) {
    ...
    if (tag_name == "__slon") 
      val = &ilVals[0];
    else if (tag_name == "slat")
      val = &jlVals[0];
    else if (tag_name == "ilev")
      val = &klVals[0];
    else
      continue;
  ...
  for (unsigned int i = 0; i != dimNames.size(); ++i) {
    if (dimNames[i] == "slon" || dimNames[i] == "slat" || dimNames[i] == "ilev") {
	...
	val[1] = lDims[3]; 
      ...
      else if (dimNames[i] == "slat") {
	val[0] = lDims[1]; 
	val[1] = lDims[4];
      }
      else if (dimNames[i] == "ilev") {
	val[0] = lDims[2]; 
	val[1] = lDims[5];
      }
...
{  
  ...
  if ((vit = std::find(dimNames.begin(), dimNames.end(), "lev")) != dimNames.end())
    ...
    ERRORR(MB_FAILURE, "Couldn't find center k variable.");
  gCDims[5] = dimVals[idx]-1;
  gCDims[2] = 0;
  kCDim = idx;
  kCName = std::string("lev");
  
  // set paraemters of i/j/k dimensions
  gDims[3] = gCDims[3]+1; 
  gDims[0] = 0;
  iName = std::string("");
  ...
  if (lDims[2] != -1) {
    if ((vmit = varInfo.find(kCName)) != varInfo.end() && (*vmit).second.varDims.size() == 1) {
      rval = read_coordinate(kCName.c_str(), lDims[2], lDims[5], klVals);
      ERRORR(rval, "Trouble reading z variable.");
    }
    else {
      ERRORR(MB_FAILURE, "Couldn't find z coordinate.");
  ...
  dbgOut.tprintf(1, "I=%d-%d, J=%d-%d, K=%d-%d\n", lDims[0], lDims[3], lDims[1], lDims[4], lDims[2], lDims[5]);
  dbgOut.tprintf(1, "%d elements, %d vertices\n", (lCDims[3]-lCDims[0]+1)*(lCDims[4]-lCDims[1]+1),
                 (lDims[3]-lDims[0]+1)*(lDims[4]-lDims[1]+1)*(lDims[5]-lDims[2]+1));

  // FIXME: ilVals right now stores coordinates for nodes, not cell centers
  // create grid specific coordinate dimension tags
  // <dim_name>
  // use slat slon 
  std::string tag_name;
