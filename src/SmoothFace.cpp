...
ErrorCode SmoothFace::find_loops()
{
  ...
  std::vector<EntityHandle> startV, endV;// these will consider senses for edges
  std::vector<int> sensesAll;
  Range::iterator it = csets.begin();
  int sense;
  for (size_t i = 0; i < totalNumEdges; i++, it++)
  {
    ...
    if (MB_SUCCESS != rval)
      return MB_FAILURE;
    EntityHandle lastEdge = entities[entities.size() - 1];// last edge in the chain of edges
    const EntityHandle * conn21;
  ...
  EntityHandle startVert = startV[0];
  //EntityHandle currentEdge = firstEdge;
  EntityHandle currentVert = endV[0];
