                               ...
                               bool normalize)
{
  ...
  ErrorCode result ;
  if (_spectralSource) {
    result = mbImpl->tag_get_handle(interp_tag.c_str(), _ntot, MB_TYPE_DOUBLE, tag);CHK_SET_ERR_STR(result, "Failed to get handle for interpolation tag \"" << interp_tag << "\"");
  }
  else {
    result = mbImpl->tag_get_handle(interp_tag.c_str(), 1, MB_TYPE_DOUBLE, tag);CHK_SET_ERR_STR(result, "Failed to get handle for interpolation tag \"" << interp_tag << "\"");
  }
                                   ...
                                   std::vector< std::vector<iBase_EntityHandle> >    *entity_groups)
{
  ...
  for (unsigned int i = 0; i < cons_tuples->get_n(); i++) {
    ...
    ERRORR("iMesh_getEntSetsByTagsRec failed.", err);
    if (debug)
      std::cout << "ent_sets_size=" << ent_sets_size << std::endl;
    ...
    for (int j = 0; j < ent_sets_size; j++) {
      ...
      if (debug)
        std::cout << "ents_size=" << ents_size << std::endl;
