...
extern "C" {
#endif
 void create_mesh (iMesh_Instance instance, iBase_EntitySetHandle * imesh_euler_set, double * icoords,
			int * icorners,
...
int main(int argc, char* argv[]){
  ...
  double * coords = (double*)malloc(nelem*(nc+1)*(nc+1)*3*sizeof(double));
  int * corners = (int*)malloc(nelem*4*sizeof(int));
  int ixc = 0, ico=0;
  for (int i=0; i<nelem; i++)
  {
	fscanf(finp, "%d %d %d %d", &corners[ixc], &corners[ixc+1], &corners[ixc+2], &corners[ixc+3]);
    ixc+=4;
	for (int i1=0; i1<=nc; i1++)
	{
		for (int j=0; j<=nc ; j++)
		{
			fscanf(finp, "%lf %lf %lf", &coords[ico], &coords[ico+1], &coords[ico+2]);
			ico+=3;
		}
	}
  }
